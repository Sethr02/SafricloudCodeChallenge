...
async function runTests() {
    ...
    await test.run('Concurrency limit enforcement', async () => {
        ...
        const promises: Promise<JobResult<number>>[] = [];
    ...
    await test.run('Rate limit enforcement', async () => {
        ...
        const timeTrackingJob = async (jobId: number): Promise<boolean> => {
            const now = Date.now();
            startTimes.push(now);
            jobIds.push(jobId);
        ...
        const promises: Promise<JobResult<boolean>>[] = [];
    ...
    await test.run('Execution order (FIFO)', async () => {
        ...
        const jobs: Promise<JobResult<number>>[] = [
    ...
    await test.run('Multiple concurrent queues', async () => {
        ...
        const jobs1: Promise<JobResult<number>>[] = [];
        const jobs2: Promise<JobResult<number>>[] = [];
